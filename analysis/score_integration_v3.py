...
for user, activities in tqdm(users_scores.items(), desc="Integrating Scores"):
    weighted_sentiment = np.mean([d['sentiment_score'] * SUBMISSION_WEIGHT for d in activities['submission']] +
                                 [d['sentiment_score'] * COMMENT_WEIGHT for d in activities['comment']])
    weighted_pos = np.mean([d.get('pos_score', 0) * SUBMISSION_WEIGHT for d in activities['submission']] +
                           [d.get('pos_score', 0) * COMMENT_WEIGHT for d in activities['comment']])
    weighted_graph = np.mean([d.get('graph_score', 0) * SUBMISSION_WEIGHT for d in activities['submission']] +
                             [d.get('graph_score', 0) * COMMENT_WEIGHT for d in activities['comment']])
    weighted_time_based = np.mean([d.get('time_based_score', 0) * SUBMISSION_WEIGHT for d in activities['submission']] +
                                  [d.get('time_based_score', 0) * COMMENT_WEIGHT for d in activities['comment']])

    user_data = {
        'sentiment': weighted_sentiment,
        'pos': weighted_pos,
        'graph': weighted_graph,
        'time_based': weighted_time_based
    }
...
with open(OUTPUT_FILE, "w", newline="") as csvfile:
    ...
    writer.writerow(["User", "Integrated_Score", "Sentiment", "POS", "Graph", "Time_Based"])
