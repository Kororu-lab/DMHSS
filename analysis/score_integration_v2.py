...
ORDER = "ascending"  # Can be "ascending" for low or "descending" for top
NORMALIZE = True  # Set to False if scores are already normalized

# MongoDB Collections
COLLECTION_SUBMISSIONS = "filtered_submissions_standard"
COLLECTION_COMMENTS = "filtered_comments_standard"
...
for user, activities in tqdm(users_scores.items(), desc="Integrating Scores"):
    # Replace with actual score aggregation logic
    avg_sentiment = np.mean([d['sentiment_score'] for d in activities['submission'] + activities['comment']])
    avg_pos = np.mean([d.get('pos_score', 0) for d in activities['submission'] + activities['comment']])
    avg_graph = np.mean([d.get('graph_score', 0) for d in activities['submission'] + activities['comment']])
    avg_time_based = np.mean([d.get('time_based_score', 0) for d in activities['submission'] + activities['comment']])
